...
def plot_rolling_stats(ts):
    ...
    orig = plt.plot(ts, color='blue', label='Original',alpha=0.5)
    mean = plt.plot(rolling_mean, color='red', label='Rolling Mean', alpha=0.5)
    std = plt.plot(rolling_std, color='black', label='Rolling Std', alpha=0.5)
    plt.legend(loc='best')
...
def arima_gridsearch_cv(series, cv_splits=2, verbose=True, show_plots=True):
    ...
    for train_index, test_index in tscv.split(series):
        ...
        if show_plots:
            ...
            # show error plot
            insample_fit = list(results.predict(train_series.index.min() + pd.Timedelta(1, unit="D") ,
                                                train_series.index.max(),
                                                typ='levels'))
            plt.plot((np.exp(train_series.iloc[1:].tolist()) - np.exp(insample_fit)))
